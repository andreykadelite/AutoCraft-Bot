...
# Resolve project root from module location when running from source.
def _source_base_dir_from_file(path: str) -> str:
    """
    Return project root for source mode.
    If this module is inside sys_core/moduls, root is one level above.
    """
    here = os.path.dirname(os.path.abspath(path))
    parent_name = os.path.basename(here).lower()
    if parent_name in ("sys_core", "moduls"):
...
if "NUITKA_ONEFILE_PARENT" in os.environ:
    ...
    BASE_DIR = _source_base_dir_from_file(__file__)
