...
def register_handlers(dp: Dispatcher):
    ...
    # Вернуться в функции (выход из режима модуля целиком)
    @error_reporter
    async def back_to_functions(message: types.Message, state: FSMContext = None):
        uid = _get_user_id(message)
        USER_STATE.pop(uid, None)
        await message.answer("Возвращаюсь к звуковым функциям.", reply_markup=get_sound_keyboard())

    dp.register_message_handler(back_to_functions, _cmd("вернуться в функции"))
