...
def register_dptools_handlers(dp, base_dir, note_mode, pending_note, file_mode, infiles_mode, power_mode, pending_power_action, get_additional_keyboard):
    ...
    async def files_menu(message: types.Message):
        ...
        await message.answer(
            "Режим отправки файлов активирован. Отправьте файл (до 20 МБ, можно отправлять любые типы файлов: документы, фото, аудио, видео и т.д.). Для завершения нажмите «Выключить режим отправки файлов».",
    ...
    @dp.message_handler(content_types=[types.ContentType.DOCUMENT, types.ContentType.PHOTO, types.ContentType.AUDIO, types.ContentType.VIDEO])
    async def handle_file_upload(message: types.Message):
        ...
        if file_size > MAX_FILE_SIZE:
            await message.answer("Файл слишком велик для отправки. Максимальный размер: 20 МБ.")
            return
