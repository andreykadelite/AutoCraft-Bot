...
def _init_logger():
    ...
    Файл лога создаётся в папке "лог" внутри base_dir (как в основном боте).
    """
...
def register_volume_handlers(dp: Dispatcher, get_sound_keyboard_cb):
    ...
    @dp.message_handler(
        lambda m:
            m.text in [
                ...
                "Вернуться в доп.меню", "На главную", "Отмена"
    ...
    async def volume_button_handler(message: types.Message):
        try:
            ...
            if text == "Отмена":
                logger.debug("volume_button_handler(): 'Отмена' вне режима выбора (chat_id=%s)", chat_id)
                if chat_id in AUDIO_OUTPUT_STATE:
                    AUDIO_OUTPUT_STATE.pop(chat_id, None)
                    await message.answer(
